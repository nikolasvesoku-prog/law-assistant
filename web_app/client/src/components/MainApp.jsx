...
export default function MainApp({ license, onLogout }) {
  ...
  const [view, setView] = useState('all') // 'all' | 'fav' | 'main' | 'codec:<id>'
  ...
  const refresh = async () => {
    ...
    try {
      const [cRes, aRes] = await Promise.all([
        api.getCodecs(),
        api.getArticles({ q, codec_id: codecId, favorites: favOnly ? '1' : '' }),
      ])
      if (reqId !== reqIdRef.current) return
      setCodecs(cRes.codecs)
      setArticles(aRes.articles)
      setFavoritesCount(aRes.favorites_count)
  ...
  const displayArticles = (() => {
    ...
    let list = ql ? articles.filter((a) => `${a.number} ${a.title} ${a.text} ${a.note} ${a.codec_name}`.toLowerCase().includes(ql)) : articles
  ...
  return (
    <div style={styles.app}>
      ...
      <main style={styles.main}>
          ...
          <>
            ...
            <div style={styles.articleList}>
              {popList.length === 0 ? (
                ...
                popList.map((a) => (
                  <ArticleBlock key={a.key} article={a} onSelect={() => setSelected(a)} onToggleFav={toggleFav} />
                ))
              )}
            </div>
          </>
        ...
        {view !== 'main' && view !== 'handbook' && view !== 'ai' && view !== 'updates' && ((view !== 'all' && view !== 'main') || q || (view === 'all' && articles.length > 0)) && (
          <>
...
// Перевод срока лишения свободы в игровые минуты (1 месяц = 1 минута).
...
function ArticleBlock({ article, onSelect, onToggleFav }) {
  ...
  return (
    <div className="card clickable" style={styles.articleCard} onClick={onSelect}>
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
        ...
        <span style={{ fontWeight: 500, flex: 1, overflow: 'hidden', textOverflow: 'ellipsis' }}>{article.title || '—'}</span>
        {canFav && (
          <button
            className="ghost"
