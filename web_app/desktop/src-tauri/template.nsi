...
; --- Тёмная тема установщика (в стиле приложения) ---
!define MUI_BGCOLOR "1A1A1A"
!define MUI_TEXTCOLOR "FFFFFF"
!define MUI_FGCOLOR "FFFFFF"
!define MUI_HEADERBACKGROUNDCOLOR "1A1A1A"
!define MUI_HEADERTEXTCOLOR "FFFFFF"
!define MUI_HEADERBUTTONCOLOR "FFFFFF"
...
!if "${INSTALLMODE}" == "both"
  ...
  !define MULTIUSER_INSTALLMODE_INSTDIR "${DISPLAYNAME}"
...
; Принудительно классические контролы: иначе текст чекбоксов (Run/Shortcut)
; не перекрашивается в белый (баг NSIS #443).
...
Function .onInit
  ...
  ${If} $INSTDIR == "${PLACEHOLDER_INSTALL_DIR}"
    ...
    !if "${INSTALLMODE}" == "perMachine"
      ${If} ${RunningX64}
        !if "${ARCH}" == "x64"
          StrCpy $INSTDIR "$PROGRAMFILES64\${DISPLAYNAME}"
        !else if "${ARCH}" == "arm64"
          StrCpy $INSTDIR "$PROGRAMFILES64\${DISPLAYNAME}"
        !else
          StrCpy $INSTDIR "$PROGRAMFILES\${DISPLAYNAME}"
        !endif
      ${Else}
        StrCpy $INSTDIR "$PROGRAMFILES\${DISPLAYNAME}"
      ${EndIf}
    !else if "${INSTALLMODE}" == "currentUser"
      StrCpy $INSTDIR "$LOCALAPPDATA\${DISPLAYNAME}"
    !endif
...
Function CreateOrUpdateStartMenuShortcut
  ; удалить старые ярлыки с прежними названиями (миграция после переименования) — всегда
  Delete "$SMPROGRAMS\$AppStartMenuFolder\law-assistant.lnk"
  Delete "$SMPROGRAMS\$AppStartMenuFolder\Правовой помощник.lnk"
  Delete "$SMPROGRAMS\law-assistant.lnk"
  Delete "$SMPROGRAMS\Правовой помощник.lnk"
  ...
  ; Skip creating shortcut if in no shortcut mode.
  ; Ярлык пересоздаём и при обновлении — чтобы после переименования он появился с новым именем.
  ${If} $WixMode = 0
    ${If} $NoShortcutMode = 1
      Return
    ${EndIf}
...
Function CreateOrUpdateDesktopShortcut
  ; удалить старые ярлыки с прежними названиями (миграция после переименования) — всегда
  Delete "$DESKTOP\law-assistant.lnk"
  Delete "$DESKTOP\Правовой помощник.lnk"
  Delete "$SMPROGRAMS\$AppStartMenuFolder\law-assistant.lnk"
  Delete "$SMPROGRAMS\$AppStartMenuFolder\Правовой помощник.lnk"
  Delete "$SMPROGRAMS\law-assistant.lnk"
  Delete "$SMPROGRAMS\Правовой помощник.lnk"
